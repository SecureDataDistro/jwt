import {handler as lambdaHandler} from "./aws_kms";
...
export class JWT implements DSSJWT{
    ...
    iat: number // issued at
    claims: string[] // list of urns
    constructor(entity: string, claims: string[]) {
        this.sub = entity;
        this.claims = claims;
        //this.iss = `sdd:hub:idenitity:${kmsKeyId}`;
    ...
    payloadRaw(keyURN:string, expiry: number) {
        this.nbf = new Date().getTime() / 1000; //seconds
        this.iat = new Date().getTime() / 1000; //seconds
        this.exp = (new Date().getTime() / 1000 ) + expiry; //seconds
        return {
